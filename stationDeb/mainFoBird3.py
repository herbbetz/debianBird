...
def luxProtocol(lData):
    camdatafile = f"{birdpath['writedir']}/camdata.json" # not suited for ramdisk/ to spare SD card, because data of several days/sessions
    data = []
    maxdata = 100
...
def send_movement(circ_output, picam, wght, trigger_ns, stop_event): # first parameter is either circ_output OR picam, the latter in case of no circ_output
    if upmaxcnt > 0 and send_movement.vid_cnt >= upmaxcnt: # upmaxcnt=0 means no limit
        ...
        subprocess.call(f"bash {birdpath['appdir']}/shutoff.sh limitdown", shell=True)
        time.sleep(2)
        return
    ...
    # for local review:
    imgName = f"{birdpath['ramdisk']}/{movementStartStr}.jpg"
    ...
    if upfail:
        ...
        subprocess.call(f"bash {birdpath['appdir']}/message.sh VideoUpfail_keepdir", shell=True)
    else:
        ...
        subprocess.call(f"bash {birdpath['appdir']}/message.sh newVideo{send_movement.vid_cnt}", shell=True)
